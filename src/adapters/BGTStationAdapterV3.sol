...
import {IInfrared} from "../utils/IInfrared.sol";
...
interface IHoneyQueen {
    ...
    function infraredBGT() external view returns (address);
}

contract BGTStationAdapterV3 is BaseVaultAdapter {
    /*###############################################################
                            STORAGE
    ###############################################################*/
    uint256[50] __gap_;
    ...
    function _claimIBGT(address vault) internal {        
        IBGTStationGauge bgtStationGauge = IBGTStationGauge(vault);
        address stakingToken = bgtStationGauge.stakeToken();
        address infrared = IHoneyQueen(honeyQueen).infrared();
        address iBGT = IHoneyQueen(honeyQueen).infraredBGT();

        if (bgtStationGauge.operator(address(this)) != infrared) {
            bgtStationGauge.setOperator(infrared);
        }

        IInfrared(infrared).claimExternalVaultRewards(stakingToken, address(this));

        uint256 iBGTBalance = IERC20(iBGT).balanceOf(address(this));
        IERC20(iBGT).transfer(locker, iBGTBalance);
