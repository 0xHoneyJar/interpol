...
contract BoycoInterpolVaultV4 is ERC20Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function rescue(address token, uint256 amount, address recipient) public onlyOwner {
        STL.safeTransfer(token, recipient, amount);
    }
    /*###############################################################
                            INTERNAL FUNCTIONS
    ###############################################################*/
    ...
    function redeem(uint256 _shares, address _receiver) public returns (uint256, uint256) {
        ...
        return (LPToWithdraw, henloToWithdraw);
    }
    /*###############################################################
                            VIEX
    ###############################################################*/
    function previewRedeem(uint256 _shares) public view returns (uint256, uint256) {
        uint256 LPBalance = locker.totalLPStaked(LPToken);
        uint256 henloBalance = IERC20(henlo).balanceOf(address(this));
        uint256 LPToWithdraw = _shares.mulDiv(LPBalance + 1, totalSupply() + 10 ** _decimalsOffset(), Math.Rounding.Floor);
        uint256 henloToWithdraw = _shares.mulDiv(henloBalance + 1, totalSupply() + 10 ** _decimalsOffset(), Math.Rounding.Floor);
